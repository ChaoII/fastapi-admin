...
def finemenu(parment):
  ...
  for p in range(len(menu)):
    ...
    routemenu['meta']['isLink'] = menu[p].link
    routemenu['meta']['isHide'] = menu[p].ishide
    routemenu['meta']['isAffix'] = menu[p].isaffix
    routemenu['meta']['isIframe'] = menu[p].isiframe
...
def getMenu(user : CurrentUser= Depends(CurrentUser)):
    ...
    data=finemenu(None)
    logger.debug(data)
    da = {"msg": "11111!","type": "adminMenu", "code": 1,"data":data}
    return da
